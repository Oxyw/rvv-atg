...
from scripts.create_test_floating.create_test_common import generate_macros_vfcvt, generate_tests_vfcvt, print_ending
import re

instr = 'vfcvt'
suffix_list_f = ['xu.f.v', 'x.f.v', 'rtz.xu.f.v', 'rtz.x.f.v']
...
def create_first_test_vfcvt(xlen, vlen, vsew, lmul, vta, vma, output_dir, rpt_path):
    ...
    # Generate tests
    num_tests_tuple = generate_tests_vfcvt(instr, suffix_list, f, lmul, rs1_val, rs2_val)
