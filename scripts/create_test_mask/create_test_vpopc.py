...
def generate_macros_vpopc(f, vsew, lmul):
    ...
    # generate the macro
    print("#define TEST_VPOPC_OP( testnum, inst,  vm_addr, mask_addr ) \\\n\
    ...
    for i in range(1, 32):
        if i == 7 or i  == 16 or i == 3 or i % lmul != 0:
            continue
        print("#define TEST_VPOPC_OP_rs2_%d( testnum, inst,  vm_addr, mask_addr ) \\\n\
            TEST_CASE_SCALAR_SETVSEW_AFTER(testnum, x14,  \\\n\
                VSET_VSEW_4AVL \\\n\
                la  x2, vm_addr; \\\n\
    ...
    for i in range(1, 32):
        if i == 7 or i  == 16 or i == 3 or i % lmul != 0 or i == 20:  # 20 is signature base 
            continue
        print("#define TEST_VPOPC_OP_rd_%d( testnum, inst,  vm_addr, mask_addr ) \\\n\
